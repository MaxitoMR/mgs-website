...
- **Two layout facts that were arrived at the hard way.** (1) The feature list is one tall column on `lg`, not two of four: two columns came out ~190px against a ~590px phone and left the row visibly bottom-heavy on the image side. (2) The phone is `justify-end` toward the centre line — a 272px phone centred in its own 580px column reads as a gap, not composition.
...
- **Known gap:** the in-frame phone screenshot (`active-shift.png`) is the app's **light** theme while the photographed phone is in **dark**. Within one continuous dark field that reads as two different products. Replacing it with a dark-theme capture of an inspection screen is the single highest-value follow-up here, and would also let the chapter show the inspection flow itself rather than a shift screen.
